    ...
    ESYS_TR        *sk_handle)
{
    ...
    try {
        ...
        rc = Esys_Create(
                        ...
                        &creation_ticket);
        check_rc(rc, "Creating symmetric key failed");

        rc = Esys_Load(
                        ctx, parent_handle, 
                        ESYS_TR_PASSWORD, ESYS_TR_NONE, ESYS_TR_NONE, 
                        out_private, out_public, &object_handle);
        check_rc(rc, "Loading symmetric key failed");
    ...
    bool          decrypt)
{
    ...
    try {
        ...
        check_rc(rc, "en/de-crypt_message: start authsession failed");

        rc = Esys_PolicyPassword(
                        ctx, session_handle, 
                        ESYS_TR_NONE, ESYS_TR_NONE, ESYS_TR_NONE);
        check_rc(rc, "en/de-crypt_message: policy password failed");
        ...
        check_rc(rc, "en/de-crypt_message: setting NV authValue failed");

        rc = Esys_PolicySecret(
                        ctx, nv_handle, session_handle,
                        ESYS_TR_PASSWORD, ESYS_TR_NONE, ESYS_TR_NONE,
                        &nonce_tpm, &cp_hash_a, &policy_ref, expiration,
                        &timeout, &policy_ticket);
        check_rc(rc, "en/de-crypt_message: policySecret failed");

        auth_value.size = sizeof(sk_password);
        memcpy(&auth_value.buffer[0], sk_password, auth_value.size);

        rc = Esys_TR_SetAuth(ctx, sk_handle, &auth_value);
        check_rc(rc, "en/de-crypt_message: setting sym key authValue failed");
        ...
        rc = Esys_EncryptDecrypt(
                        ...
                        &iv_in, &in_data, 
                        &out_data, &iv_out);
        check_rc(rc, "en/de-crypt_message: encryptdecrypt failed");

        rc = Esys_TR_SetAuth(ctx, nv_handle, nullptr);
        check_rc(rc, "en/de-crypt_message: clearing nv password from memory failed");
        
        rc = Esys_TR_SetAuth(ctx, sk_handle, nullptr);
        check_rc(rc, "en/de-crypt_message: clearing sym key password from memory failed");

        rc = Esys_TR_Close(ctx, &session_handle);
        check_rc(rc, "en/de-crypt_message: flushcontext failed");
